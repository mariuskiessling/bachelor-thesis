...
in a separate \texttt{architecture} repository. Therefore, a microservice's
code and its deployment manifests are fully isolated. Each manifest file holds
all the microservice's Kubernetes objects, e.g.\ Deployments, Services or
...
Additionally a set of configurations are defined next to \circled{2}. These
...
\ac{CIST} was developed with automation in mind. However, \ac{CI} was not only
...
With continuous versioning, one version of a master or component chart can
...
Figure~\ref{fig:cist_chart} shows the sequence of steps that are executed when
