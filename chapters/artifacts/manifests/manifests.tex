...
in a separate \texttt{deployment} repository. So a microservice's code and its
deployment manifests are fully isolated. Furthermore, up to this point no
distinction has been made between services that either rely on a database or do
not need one. Yet, services that depend on a database have a second Kubernetes
manifest that holds the databases' configuration.

During a services deployment, the pipeline reads the service's Kubernetes
manifest and applies it to the cluster.
