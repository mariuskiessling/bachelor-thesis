...
\subsection{Evaluating Automatic Versioning}%
\label{sub:Evaluating_Automatic_Versioning}
To evaluate the usefulness of automatic versioning schemes (like CalVer), a
number of requirements have to be met. For the purpose of traceability these
requirements are labelled $R_1$ to $R_n$.

\begin{itemize}
  \item $R_1$: Automatic versioning must be applicable to a microservice
    architecture.
  \item $R_2$: Automatic versioning must be applicable to all components of a
    microservice (packaged software, the software itself, container image and
    \ac{API}).
  \item $R_3$: Automatic versioning must be compatible with \ac{CI} systems.
  \item $R_4$: Automatic versioning must be compatible with \ac{CD} systems.
  \item $R_5$: Automatic versioning should improve the performance of the team.
\end{itemize}
...
microservice architecture can be versioned using an automatic versioning
...
can receive version numbers. The chapter introduced ways to apply automatic
...
automatically versioned component, e.g.\ a Node.js software. Container images
get versioned by applying the automatic version number as a tag. Lastly, both
...
Not only is automatic versioning compatible with a \ac{CI} system, the \ac{CI}
system is an integral component that makes automatic versioning work. The
...
differently than a automatically versioned one. Thus, as long as the \ac{CD}
system can continuously deploy a manually versioned microservice, it can do the
same for an automatically versioned one and thus gratified $R_4$.
...
\subsection{Economic Benefits of Automatic Versioning}%
\label{sub:Economic_Benefits_of_Automatic_Versioning}
...
thus eliminating additional operations teams. Automatic versioning takes this
...
backward compatibility) an automatic versioning scheme can not be used. In the
