...
build process for containers. The outcome of such a build process is a
...
1]{LuksaKubernetesAction2017}. A container is an instantiation of an image. A
container is spawned based on one image; yet one image can be the base for
multiple containers.
...
fully deterministic. The issues in respect to the deterministic build process
will also be discussed in chapter TODO.
...
which is one of the smallest base images available. % TODO: Add citation!
...
produces the output \\
\hspace{0.5em}\texttt{Hello from BusyBox \textbf{VERSION\textunderscore{}NUMBER}} \hspace{0.4em}.\\
...
interrelations are however fully contested in
...
a few problems:
\begin{itemize}
  \item The developer has to tell the production machine running Docker to run
    a new image by hand.
