...
service's connection or the service itself deals with it. To decide which layer
fits the situation at hand best, it has to be distinguished between
applications which enter a state they can not recover from when their
connection to the database is dropped and applications which can recover from
such a state.
