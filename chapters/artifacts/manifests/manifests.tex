...
fully autonomous continuous deployment process. The issues include:

\begin{itemize}
  \item A \ac{CI} process is only started when changes are merged from the
  ...
  \item When deploying $\mu$App from scratch, the deployment is comprised of
    three steps that need to be executed by hand.
  \item The deployment of the actual microservice and its database is decoupled
    and thus are executed independently from one another.
  \item A microservice's database has to be deployed first to guarantee its
    available once the actual microservice using that database is deployed.
  \item A microservice's manifest files are managed independently from the
...
\begin{itemize}
  \item The second a microservice looses the connection to its database, it
...
section~\ref{ssub:Deploying_to_Kubernetes_The_Naive_Way}.
